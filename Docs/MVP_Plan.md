...
AI-DAY-T23-AUTONOMOUS-QUOTE-AGENTS/
  ...
  ML/
    ...
    notebooks/
      ...
      03_Agent1_Risk_Profiler.ipynb
      04_Agent2_Conversion_Predictor.ipynb
      05_Regional_Intelligence.ipynb
    models/
...
## 11. Quote Input Modes

The dashboard supports two input modes:

### Mode 1 — Custom Quote Form

A structured form where a user (agent, underwriter, or demo judge) enters a single quote manually:

| Field | Input Type | Values |
|---|---|---|
| Agent Type | Dropdown | EA, IA |
| Region | Dropdown | A–H |
| Driver Age | Number slider | 16–85 |
| Driving Experience | Number slider | 0–65 |
| Previous Accidents | Number stepper | 0–5 |
| Previous Citations | Number stepper | 0–5 |
| Household Vehicles | Number stepper | 1–9 |
| Household Drivers | Number stepper | 1–9 |
| Coverage | Dropdown | Basic, Balanced, Enhanced |
| Salary Range | Dropdown | 5 bands |
| Vehicle Cost Range | Dropdown | 5 bands |
| Vehicle Usage | Dropdown | Pleasure, Commute, Business |
| Annual Miles Range | Dropdown | 7 bands |
| Quoted Premium | Number input | $200–$2000 |
| Re-Quote | Toggle | Yes / No |
| Gender | Dropdown | Male, Female |
| Marital Status | Dropdown | Single, Married, Divorced, Widow |
| Education | Dropdown | High School, College, Bachelors, Masters, Ph.D |
| Policy Type | Dropdown | Car, Van, Truck |

On submit: `POST /api/process-quote` with the form data. Quote_Num is auto-generated (e.g. `CUSTOM-001`). Q_Creation_DT = today, Q_Valid_DT = today + 59 days.

### Mode 2 — Sample Batch

- "Load N Samples" button (default 10, max 50)
- Calls `GET /api/sample-quotes?n=10` to pull random rows from the dataset
- Displays them in a preview table for inspection
- "Process All" sends them to `POST /api/process-batch`
- Results stream in via SSE and appear one by one in the live table

---

## 12. Pipeline Process Flow & Live Status

Each quote shows a **visual pipeline stepper** that updates in real time as it passes through each agent. This is the key demo moment — judges see the pipeline "thinking" step by step.

### Stepper UI

```
[ Agent 1: Risk Profiler ] → [ Agent 2: Conversion ] → [ Agent 3: Premium ] → [ Agent 4: Decision ]
       ✓ COMPLETE              ● PROCESSING              ○ PENDING              ○ PENDING
       risk_tier: HIGH         bind_score: ...            ...                    ...
```
...
|---|---|---|
| PENDING | Gray circle | Agent hasn't started |
| PROCESSING | Pulsing blue dot | Agent is running |
| COMPLETE | Green check | Agent finished, output available |
| SKIPPED | Dash | Agent 3 skipped (bind_score <= 60) |

### Per-Agent Explanation Panels

When an agent completes, its explanation panel expands below the stepper showing:

**Agent 1 — Risk Profiler:**
- Risk tier badge (LOW/MEDIUM/HIGH with color)
- SHAP: top 3 features as horizontal bar chart (green = pushes LOW, red = pushes HIGH)
- LIME: local feature weights as a mini chart
- Anchors: IF-THEN rule displayed as a highlighted text block
- DiCE: "To change risk to LOW, change X, Y, Z" as an actionable card

**Agent 2 — Conversion Predictor:**
- Bind score as a circular progress gauge (0–100)
- Urgency: "X days until quote expires" with color (red if < 14 days)
- SHAP: top 3 features for bind prediction
- LIME: local weights
- Anchors: rule for bind/no-bind
- DiCE: "This quote would convert if premium was $X instead of $Y" as actionable cards

**Agent 3 — Premium Advisor (or SKIPPED):**
- Premium flag badge (BLOCKER / ACCEPTABLE)
- Adjusted price band visualization
- LLM reasoning text block (chain-of-thought)

**Agent 4 — Decision Router:**
- Final decision badge (AUTO_APPROVE / AGENT_FOLLOWUP / ESCALATE_UNDERWRITER)
- Structured case summary (all upstream signals compiled)
- For escalated quotes: LLM-generated narrative summary

---

## 13. Dashboard — Key Screens

### Screen 1 — Quote Input + Live Pipeline
- Top: Toggle between "Custom Quote" form and "Load Samples" mode
- Middle: Pipeline stepper with live status per agent (the core demo visual)
- Bottom: Expandable explanation panels for each completed agent

### Screen 2 — Processed Quotes Table
- All processed quotes in a sortable/filterable table
...
- Click any row to expand and see full explanation details from all 4 agents
...
### Screen 5 — Regional & Channel Intelligence (Bonus)
...
## 14. Agent 3 — Groq Llama 3.3 70B Prompt Template
...
| Method | Path | Purpose |
|---|---|---|
| POST | `/api/process-quote` | Process single quote (custom form or selected sample) through 4-agent pipeline |
| POST | `/api/process-batch` | Process multiple sample quotes, streams results via SSE |
| GET | `/api/stream` | SSE endpoint — pushes per-agent status updates + final results to connected clients |
...
| GET | `/api/form-options` | Returns dropdown options for the custom quote form (regions, coverages, salary bands, etc.) |

### SSE Event Types for Live Pipeline Status

The `/api/stream` endpoint pushes structured events so the frontend stepper updates in real time:

```json
// Agent starts processing
{"event": "agent_start", "data": {"quote_id": "Q-12345", "agent": 1, "agent_name": "Risk Profiler"}}

// Agent completes with its output + explanations
{"event": "agent_complete", "data": {"quote_id": "Q-12345", "agent": 1, "agent_name": "Risk Profiler", "output": {"risk_tier": "HIGH", "risk_score": 0.87, "shap_top3": [...], "lime": {...}, "anchor_rule": "IF ...", "counterfactuals": [...]}}}

// Agent skipped (Agent 3 when bind_score <= 60)
{"event": "agent_skipped", "data": {"quote_id": "Q-12345", "agent": 3, "reason": "bind_score <= 60"}}

// Full pipeline complete
{"event": "pipeline_complete", "data": {"quote_id": "Q-12345", "decision": "AUTO_APPROVE", "full_result": {...}}}
```

---

## 16. Groq Free Tier Limits
...
## 18. Regional & Channel Intelligence (Bonus — Required)
